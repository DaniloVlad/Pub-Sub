...
class Channel extends AbstractChannel {
	...
	public Channel(String channelTopic) {
		...
		System.out.println("Channel :: " + this + " has been created!");
	}
	...
	private void notifySubscribers(AbstractEvent event) {
		System.out.println("Channel :: " + this + " is notifying its subscribers!");
		AbstractEvent currentEvent; 
		currentEvent = event;
		for(AbstractSubscriber subscriber : subscribers) {
			//access control line dont notify blocked subs
			if(! ChannelAccessControl.getInstance().checkIfBlocked(subscriber, this.channelTopic))
				subscriber.alert(currentEvent, this.channelTopic);
			else
				System.out.println("Channel :: " + subscriber + " is blocked on channel: " + this.channelTopic);
		}
	}
