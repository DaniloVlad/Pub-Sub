...
public abstract class AbstractSubscriber implements IEntity {
	//NOTE: this is an abstract class hence no implementation of 
	//the methods should be here. Acts more like an interface however
	//we can extend it to encompass some default behavior incase
	//for subclasses that dont implement *all* the methods.
	
	protected IState state;
	protected Queue<AbstractEvent> completed = new LinkedList<AbstractEvent>();
	protected Queue<AbstractEvent> notCompleted = new LinkedList<AbstractEvent>();;
	...
	public Queue<AbstractEvent> getCompletedEvents() {
		return completed;
	}
	public Queue<AbstractEvent> getEvents() {
		return notCompleted;
	}
	
	public AbstractEvent popNextEvent() {
		if(notCompleted.isEmpty())
			return null;
		else {
			return notCompleted.remove();
		}
	}
	/**
